...
def plot_hist_sims(args, model, ds_test):
    ...
    ax[0].hist(neg_sims, label='neg', weights=1 / neg_sims, alpha=0.5)
    ax[0].hist(class_sims, label='class', weights=1 / class_sims, alpha=0.5)
    ax[0].hist(inst_sims, label='inst', weights=1 / inst_sims, alpha=0.5)
    ax[0].legend()

    ax[1].set_title('projected similarity types')
    ax[1].hist(proj_neg_sims, label='neg', weights=1 / proj_neg_sims, alpha=0.5)
    ax[1].hist(proj_class_sims, label='class', weights=1 / proj_class_sims, alpha=0.5)
    ax[1].hist(proj_inst_sims, label='inst', weights=1 / proj_inst_sims, alpha=0.5)
    ax[1].legend()
